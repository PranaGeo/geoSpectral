...
setMethod("spc.interp.spectral", signature = "Spectra", 
		def = function (source1,target_lbd,show.plot=FALSE){
			...
			DF=sapply(1:nrow(DF), function(x) {
						my = approx(spc.getwavelengths(source1), source1@DF[1,],xout=target_lbd)
						DF[x,] = t(my$y)
					})
			DF = data.frame(t(DF))
			if(show.plot){
				plot(spc.getwavelengths(source1), source1@DF[1,],type="l",ylab=source1@LongName,xlab="Wavelength")
